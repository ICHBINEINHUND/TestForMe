...
public class BillService {
  ...
  public List<Bill_Entity> getBillByCombinedCondition(LocalDateTime dateExport, String typeDate, String id, String phone, String add, String idUser, EnumType.Status_Bill statusBill, String sortField, String sortOrder, int setOff) {
    ...
    for (int i = 0; i < 3; i++) {
      futures.add(CompletableFuture.supplyAsync(() -> {
        ...
        while (continueFlag.get()) {
          ...
          List<Bill_Entity> partialResult = billDao.getFilteredBills(dateExport, typeDate, id, phone, add, idUser, statusBill, sortField, sortOrder, offset, setOff);
    ...
    List<Bill_Entity> results = futures.stream().map(CompletableFuture::join).flatMap(List::stream).collect(Collectors.toList());
    executor.shutdown();
    return results;
  }
